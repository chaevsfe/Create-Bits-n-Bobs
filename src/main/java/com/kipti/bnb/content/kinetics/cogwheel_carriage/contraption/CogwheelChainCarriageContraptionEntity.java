...
public class CogwheelChainCarriageContraptionEntity extends OrientedContraptionEntity {
    ...
    public CogwheelChainCarriageContraptionEntity(final EntityType<? extends OrientedContraptionEntity> type, final Level level) {
        super(type, level);
    }
    ...
    private void tickServerSync(final CogwheelChainAttachment attachment) {
        if (this.tickCount % 60 == 0) {
            BnbNetwork.sendToClientsTrackingEntity(
                    this,
...
//            CreateBitsnBobs.LOGGER.info("Client too far from server! Correcting. Diff: {}", clientServerDiff);
    ...
    @Override
    protected void writeAdditional(final ValueOutput compound, final boolean spawnPacket) {
    ...
    @Override
    protected void readAdditional(final ValueInput compound, final boolean spawnPacket) {
        super.readAdditional(compound, spawnPacket);
        if (!Float.isNaN(compound.getFloatOr("ChainAttachmentDist", Float.NaN))) {
            final float attachmentDist = compound.getFloatOr("ChainAttachmentDist", 0f);
