...
import com.kipti.bnb.mixin.client.chain.ChainConveyorRidingHandlerAccessor;
...
public final class CogwheelChainRidingHelper {
    ...
    /** Drops the riding state without telling the server, for use when the connection is already gone. */
    ...
    public static void disembark() {
        ...
        if (attachment != null) {
            BnbNetwork.sendToServer(new CogwheelChainRidingPacket(attachment.getControllerPos(), true));
        }
    ...
    public static void disembarkFromAnyPreviousRide() {
        if (ChainConveyorRidingHandler.ridingChainConveyor != null) {
            ChainConveyorRidingHandlerAccessor.bits_n_bobs$invokeStopRiding(Minecraft.getInstance());
        }
    ...
    public static void clientTick(final LocalPlayer player) {
        ...
        if (AnimationTickHolder.getTicks() % 10 == 0) {
            BnbNetwork.sendToServer(new CogwheelChainRidingPacket(attachment.getControllerPos(), false));
        }
    }
    ...
    private static Vec3 computeHangPosition(final LocalPlayer player) {
        ...
        return player.position().add(0, player.getBoundingBox().getYsize() + chainYOffset, 0);
    }
