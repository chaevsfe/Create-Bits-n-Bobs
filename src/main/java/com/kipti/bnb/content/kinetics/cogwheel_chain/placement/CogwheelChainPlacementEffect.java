...
public final class CogwheelChainPlacementEffect {

    private CogwheelChainPlacementEffect() {
    }
    ...
    public static void tick(final LocalPlayer player) {
        ...
        if (level == null || getCurrentChainLevel() == null || getCurrentBuildingChain() == null
                || getCurrentChainType() == null || getCurrentChainItemType() == null) {
            return;
        }

        if (!getCurrentChainLevel().equals(level.dimension())
                || getCurrentBuildingChain().checkMissingNodesInLevel(level, getCurrentChainType())) {
            clearPlacingChain();
            return;
        }
        ...
        final double additionalDistance = targetedPos != null
                ? Vec3.atLowerCornerOf(targetedPos.subtract(getCurrentBuildingChain().getLastNode().pos())).length()
                : 0;
        ...
        final boolean hasEnough = ChainConveyorBlockEntity.getChainsFromInventory(
                player, getCurrentChainItemType().getDefaultInstance(), chainsRequired, true);
    ...
    /**
     * Target is a candidate cogwheel: check connection validity. Valid to outliner with connections only (no block
     * shape). Invalid to red particles from last node to target.
     */
    private static @Nullable BlockPos displayTargetCandidate(final ClientLevel level,
        ...
        if (!getCurrentChainType().getCogwheelPredicate().test(targetedState.getBlock())) {
            ChainDriveDisplayRenderer.renderParticlesBetween(level, lastNode.center(), Vec3.atCenterOf(targetedPos), INVALID_COLOUR);
            return null;
        }
        ...
        try {
            PlacingCogwheelChain.validateConnection(lastNode, targetNode, previousNode, getCurrentChainType());
            renderTargetConnection(lastNode, targetNode);
            return targetedPos;
        } catch (final ChainInteractionFailedException expected) {
            return null;
        }
    }
    ...
    private static void displayProjectedTarget(final ClientLevel level,
        ...
        final Vec3 axisNormal = Vec3.atLowerCornerOf(Direction.get(Direction.AxisDirection.POSITIVE, axis).getUnitVec3i());
        ...
        final int colour = distance <= BnbServerSettings.cogwheelMaxBounds() ? VALID_COLOUR : INVALID_COLOUR;
