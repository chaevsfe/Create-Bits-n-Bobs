...
public class CogwheelChain {
    ...
    public CogwheelChain(final CompoundTag tag) {
        ...
        this.type = BnbCogwheelChainTypes.CHAIN;
        this.returnedItem = Items.IRON_CHAIN;
        this.read(tag);
    }

    public void read(final CompoundTag tag) {
        this.cogwheelNodes.clear();
        final int cogWheelPosCount = tag.getIntOr("cogwheel_pos_count", 0);
        for (int i = 0; i < cogWheelPosCount; i++) {
            final CompoundTag posTag = tag.getCompoundOrEmpty("cogwheel_pos_" + i);
        ...
        if (tag.contains("chain_type")) {
            final Identifier typeId = Identifier.parse(tag.getStringOr("chain_type", ""));
            final CogwheelChainType foundType = BnbRegistries.COGWHEEL_CHAIN_TYPES.getValue(typeId);
            if (foundType != null) {
                this.type = foundType;
            }
        }
        if (tag.contains("returned_item")) {
            final Identifier itemId = Identifier.parse(tag.getStringOr("returned_item", ""));
            final Item foundItem = BuiltInRegistries.ITEM.getValue(itemId);
    ...
    public AABB getRenderBounds() {
        if (this.cachedRenderBounds != null)
            return this.cachedRenderBounds;
        AABB bounds = null;
        ...
        this.cachedRenderBounds = bounds == null ? null : bounds.inflate(0.5);
        return this.cachedRenderBounds;
    }
