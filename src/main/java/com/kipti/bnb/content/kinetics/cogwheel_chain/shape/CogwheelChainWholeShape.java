...
public class CogwheelChainWholeShape extends CogwheelChainShape {
    ...
    @Override
    protected void drawOutline(final PoseStack.Pose transform, final VertexConsumer vb, final UnaryOperator<Vec3> positionTransform) {
        ...
        for (int i = 0; i < this.segmentCount; i++) {
            ...
            for (int cornerIndex = 0; cornerIndex < 4; cornerIndex++) {
                line(vb, transform, startCorners.get(cornerIndex), endCorners.get(cornerIndex), color);
            }

            drawRing(vb, transform,
                    startCorners.get(0),
                    startCorners.get(1),
                    startCorners.get(2),
                    startCorners.get(3),
    ...
    private static void drawRing(final VertexConsumer vb, final PoseStack.Pose transform,
                                 final Vec3 a, final Vec3 b, final Vec3 c, final Vec3 d,
                                 final int color) {
        line(vb, transform, a, b, color);
        line(vb, transform, b, c, color);
        line(vb, transform, c, d, color);
        line(vb, transform, d, a, color);
    }
    ...
    private static void line(final VertexConsumer vb, final PoseStack.Pose transform,
