...
public class FlywheelMovementMechanics {
    ...
    public static float getTransferCapacityPerAngularMass() {
        return BnbServerSettings.flywheelTransferCapacityPerAngularMass();
    }
    ...
    public static float getConfiguredMaxAngularVelocity() {
        final float factor = BnbServerSettings.flywheelMaxRpmFactor();
     ...
     */
    public void writeAdditional(final ValueOutput compound) {
//        compound.putFloat("MaxAngularVelocity", maxAngularVelocity);
     ...
     */
    public void readAdditional(final ValueInput compound, final boolean clientPacket) {
//        if (compound.contains("MaxAngularVelocity"))
//            maxAngularVelocity = compound.getFloatOr("MaxAngularVelocity", 0f);
        if (compound.contains("AngularVelocity"))
            angularVelocity = compound.getFloatOr("AngularVelocity", 0f);
        ...
        angularMass = compound.getFloatOr("AngularMass", 0f);
    }
    ...
    public void assemble(final FlywheelBearingBlockEntity be, final BearingContraption contraption) {
        angularMass = 0f;
        final Vec3 axis = Vec3.atLowerCornerOf(contraption.getFacing().getUnitVec3i());
        ...
        for (final Map.Entry<BlockPos, StructureTemplate.StructureBlockInfo> block : contraption.getBlocks().entrySet()) {
            final float angularDistance = (float) Vec3.atCenterOf(block.getKey()).subtract(anchor).cross(axis).length() + 1f;
    ...
    public static float getActualStressUnitsPerKeJoule() {
        return (float) (STRESS_UNITS_PER_KE_JOULE * BnbServerSettings.flywheelStorageFactor());
    }
