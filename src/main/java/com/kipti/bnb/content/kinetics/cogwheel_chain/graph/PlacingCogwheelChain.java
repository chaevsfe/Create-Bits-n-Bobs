...
public class PlacingCogwheelChain {
    ...
    public int getChainsRequired(double length, final CogwheelChainType type) {
        final float factor = BnbServerSettings.cogwheelChainDriveCostFactor();
        if (factor == 0) {
            return 0;
        }
    ...
    public boolean tryAddNode(final BlockPos newPos,
        ...
        if (this.getSize() >= BnbServerSettings.cogwheelMaxNodeCount() && !newPos.equals(this.getFirstNode().pos())) {
    ...
    public Vec3 getNodeCenter(final int i) {
        return Vec3.atCenterOf(this.visitedNodes.get(i).pos());
    }
    ...
    public boolean exceedsMaxBounds(final PlacingCogwheelNode candidate) {
        ...
        return newMaxBounds > BnbServerSettings.cogwheelMaxBounds();
    }

    public static float getCogwheelMaxInteractionDistanceSq() {
        return 3.0f * (BnbServerSettings.cogwheelMaxBounds() + 10) * (BnbServerSettings.cogwheelMaxBounds() + 10);
    }
