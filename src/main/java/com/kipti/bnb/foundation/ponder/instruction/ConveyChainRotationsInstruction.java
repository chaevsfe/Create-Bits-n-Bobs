...
public class ConveyChainRotationsInstruction extends PonderInstruction {
    ...
    public void tick(final PonderScene scene) {
        final BlockEntity blockEntity = scene.getLevel().getBlockEntity(chainStartLocation);
        ...
        final CogwheelChainBehaviour behaviour = CogwheelChainBehaviour.getOrThrow(scene.getLevel(), chainStartLocation, CogwheelChainBehaviour.TYPE);
        if (behaviour.isController()) {
            ...
            final BlockEntity controllerBlockEntity = scene.getLevel().getBlockEntity(chainStartLocation.offset(offset));
            ...
            final CogwheelChainBehaviour controllerBehaviour = behaviour.getSameBehaviourOrThrow(chainStartLocation.offset(offset));
    ...
    private void conveyRotationsFromController(final CogwheelChainBehaviour controllerBehaviour, final KineticBlockEntity kineticBlockEntity, final float rpm) {
        ...
        for (final PathedCogwheelNode chainNode : new ArrayList<>(chain.getChainPathCogwheelNodes())) {
            ...
            final CogwheelChainBehaviour behaviour = controllerBehaviour.getSameBehaviourOrThrow(nodePos);
    ...
    private void modifyBlockEntityKineticRotation(final Level level, final KineticBlockEntity childCogwheelChainBlockEntity, final float initialFactor, final float factor, final float rpm) {
        ...
        childCogwheelChainBlockEntity.setSpeed(rpm * initialFactor / factor);
    }
