...
public class DyedTankScenes {
    ...
    private static void applyColorToRegion(
        ...
        final Level level = ponderScene.getLevel();
        for (int x = x1; x <= x2; x++) {
            for (int y = y1; y <= y2; y++) {
                for (int z = z1; z <= z2; z++) {
    ...
    private static void splitTankMultiBlock(final CreateSceneBuilder scene, final BlockPos... positions) {
        ...
        scene.addInstruction(ponderScene -> {
            for (final BlockPos pos : positions) {
                if (ponderScene.getLevel().getBlockEntity(pos) instanceof final FluidTankBlockEntity tankBE) {
                    final CompoundTag nbt = PonderBlockEntityIO.save(tankBE, ponderScene.getLevel());
                    nbt.remove("Controller");
                    nbt.remove("Uninitialized");
                    nbt.putInt("Size", 1);
                    nbt.putInt("Height", 1);
                    nbt.putBoolean("Window", true);
                    PonderBlockEntityIO.load(tankBE, nbt, ponderScene.getLevel());
                }
            }
            ponderScene.forEach(WorldSectionElement.class, WorldSectionElement::queueRedraw);
        });
    ...
    private static void showPipeSequence(
        ...
        for (int i = 0; i < positions.length; i++) {
            ...
            if (i < positions.length - 1) {
                ...
                direction = Direction.getNearest(delta.getX(), delta.getY(), delta.getZ(), Direction.DOWN);
            }
            scene.idle(4);
        }
    }
        ...
        scene.addInstruction(ponderScene -> {
            for (final BlockPos pos : positions) {
                DyeablePipeBehaviour.refreshPipeState(
                        ponderScene.getLevel(), pos, ponderScene.getLevel().getBlockState(pos), false
                );
            }
            ponderScene.forEach(WorldSectionElement.class, WorldSectionElement::queueRedraw);
        });
